...
parser.add_argument('-w', help='window size in kb', type=int)
parser.add_argument('-s', help='step size in kb', type=int)
parser.add_argument('-o', help='outpath', type=str)
args = parser.parse_args()

window_size = args.w*1000
step_size = args.s*1000
...
female_ids = metadata_df.loc[metadata_df.GENETIC_SEX == "F"].GVCF_ID
female_samples = [True if x in list(female_ids) else False for x in ds_full.sample_id.values]
...
for c, c_ID in zip(kept_contigs, contig_IDs):
    ...
    ds_cohort_d = ds_cohort.sel(variants = ((ds_cohort.cohort_allele_count[:,0] >= 1).sum(axis=1) >= 2).compute())
    ds_cohort_d = sg.window_by_position(ds_cohort_d, size=window_size+step_size, step=step_size) 
    chrom_df = pd.DataFrame({"window_start": list(range(0, len(ds_cohort_d.window_start)*step_size, step_size))})
    if len(ds_cohort_d.variants) < 10:
        chrom_df["Tajimas_D"] = np.nan
    else:
        ds_cohort_d = (sg.Tajimas_D(ds_cohort_d))
    chrom_df["chrom"] = c
    chrom_df["Tajimas_D"] = ds_cohort_d.stat_Tajimas_D
    df_l.append(chrom_df)
df_tajima_d = pd.concat(df_l)
intervals_callable = pos_windows(bed_files, step_size, df_tajima_d["chrom"].unique())
intervals_callable["window_center"] = intervals_callable.window_start
intervals_callable.pop("window_start")
df_tajima_d["window_center"] = df_tajima_d.window_start+window_size//2
df_tajima_d["window_center_end"] = df_tajima_d.window_start+window_size//2+step_size
df_tajima_d["chr_type"] = df_tajima_d["chrom"].map(dict(zip(regions_df.CONTIG_ID, regions_df.chr_type)))
output_df = pd.merge(df_tajima_d, intervals_callable, on=["chrom", "window_center"])
output_df["feature_name"] = output_df["chr_type"]+":"+output_df["callable_frac"].astype(str)
output_df["species"] = long_form
output_df.to_csv("{}{}_{}kb_{}step_Tajima_D.txt".format(args.o, long_form, args.w, args.s), sep="\t", index=False)
bed_df = output_df[["chrom", "window_center", "window_center_end", "feature_name", "Tajimas_D"]]
bed_df.to_csv("{}{}_{}kb_{}step_Tajima_D.bed".format(args.o, long_form, args.w, args.s), sep="\t", index=False, header=False)
